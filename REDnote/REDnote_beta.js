...
function handleManualTrigger() {
  ...
  $persistentStore.write("true", key); // For Panel/Shortcut method
  $notification.post(
    "小红书表情下载模式", 
    "✅ 模式已开启", 
    "请返回小红书，下拉刷新评论区即可下载。该模式在刷新后会自动关闭。"
  );
  $done();
}
...
function handleHttpRequest() {
  ...
  if (url.includes("/api/sns/v5/note/comment/list?") || url.includes("/api/sns/v3/note/comment/sub_comments?")) {
    ...
    // 【修正】直接安全地检查Loon传入的全局变量 $argument
    const downloadModeEnabled = (typeof $argument !== 'undefined' && $argument === 'true');

    if (downloadModeEnabled) {
        console.log('[小红书] Loon UI开关检测到: [开启]，正在转换表情包为图片...');
    }
