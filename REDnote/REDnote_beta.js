...
function handleManualTrigger() {
  $notification.post("小红书脚本", "提示", "此功能为Surge面板专用。");
  $done();
}
...
function handleHttpRequest() {
  ...
  if (url.includes("/search/hint")) obj.data.hint_words = [];
  ...
  if (url.includes("/v4/note/videofeed") || url.includes("/v3/note/videofeed?")) {
    let items = url.includes("/v4/note/videofeed") ? obj.data : obj.data; // v3 and v4 have similar structures
    if (items?.length > 0) {
      ...
      // Specific logic for v4 video feed URL caching
      if (url.includes("/v4/note/videofeed")) {
        ...
        for (let item of obj.data) {
          ...
          if (Array.isArray(h265List) && h265List.length > 0) {
            ...
            if (sortedList.length > 0 && item.id) {
              videoData.push({ id: item.id, url: sortedList[0].master_url });
            }
          }
        }
  ...
  if (url.includes("/v10/note/video/save")) {
    ...
    if (obj.data?.note_id && videoFeed.length > 0) {
      const videoInfo = videoFeed.find(v => v.id === obj.data.note_id);
      if (videoInfo) obj.data.download_url = videoInfo.url;
    }
    if (obj.data?.disable) {
      obj.data = { ...obj.data, status: 2, disable: undefined, msg: undefined };
    }
  }
  ...
  if (url.includes("/v4/followfeed")) {
    if (obj?.data?.items?.length > 0) {
      obj.data.items = obj.data.items.filter((i) => i.recommend_reason !== "recommend_user");
    }
  }
  if (url.includes("/recommend/user/follow_recommend")) {
    if (obj?.data?.title === "你可能感兴趣的人") obj.data = {};
  }
  if (url.includes("/v6/homefeed")) {
    if (obj?.data?.length > 0) {
      obj.data = obj.data.filter(item =>
        !item.ads_info && !item.card_icon && !item.note_attributes?.includes("goods") && item.model_type !== "live_v2"
      );
    }
  }
  ...
  if (url.includes("/api/sns/v5/note/comment/list?") || url.includes("/api/sns/v3/note/comment/sub_comments?")) {
    ...
    // 【最终修正】检查 $argument 字符串是否包含 "enable=true"
    const downloadModeEnabled = (typeof $argument !== 'undefined' && String($argument).includes('enable=true'));

    if (downloadModeEnabled) {
        console.log(`[小红书] Loon UI开关检测到: [开启]，参数为: ${$argument}，正在转换表情包...`);
    }
  ...
  if (url.includes("/api/sns/v1/interaction/comment/video/download?")) {
    ...
    if (commitsCache) {
      ...
      if (commitsRsp.livePhotos.length > 0 && obj.data?.video) {
        const videoInfo = commitsRsp.livePhotos.find(v => v.videId === obj.data.video.video_id);
        if (videoInfo) obj.data.video.video_url = videoInfo.videoUrl;
      }
    }
  }
...
function replaceRedIdWithFmz200(obj) {if (Array.isArray(obj)) {obj.forEach(item => replaceRedIdWithFmz200(item));} else if (typeof obj === 'object' && obj !== null) {if ('red_id' in obj) {obj.fmz200 = obj.red_id;delete obj.red_id;}Object.keys(obj).forEach(key => replaceRedIdWithFmz200(obj[key]));}}
