...
if (url.includes("/api/sns/v5/note/comment/list?") || url.includes("/api/sns/v3/note/comment/sub_comments?")) {
  // --- 功能：读取Loon UI开关状态 ---
  const isEmojiToPicEnabled = $.getdata("fmz200.xiaohongshu.comments.emojiToPic") !== 'false';
  if (isEmojiToPicEnabled) {
    console.log('评论区表情包转图片功能：已开启');
  } else {
    console.log('评论区表情包转图片功能：已关闭');
  }
...
function imageEnhance(jsonStr) {
  ...
  // --- 从 $.argument 读取画质设置 (兼容性修复) ---
  const argument = $.argument || ''; // 获取完整的 argument 字符串
  console.log(`读取到的Argument为: ${argument}`);

  // 检查 argument 中是否包含 quality=original 的设置
  if (argument.includes('quality=original')) { // 原始分辨率，PNG格式的图片，占用空间比较大
    console.log("画质修改为-原始分辨率 (来自Argument)");
    jsonStr = jsonStr.replace(/\?imageView2\/2[^&]*(?:&redImage\/frame\/0)/g, "?imageView2/0/format/png&redImage/frame/0");
  } else { // 默认或任何其他情况，都使用高像素输出
    console.log("画质修改为-高像素输出 (默认)");
