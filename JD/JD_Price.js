...
$.log(`读取参数: ${args}`);
const argObj = Object.fromEntries(args.split("&").filter(item => item.includes("=")).map((item) => item.split("=").map(decodeURIComponent)));
  ...
  $.appType = currentReqUrl.includes("lite-in.m.jd.com") ? "jdtj" : "jd";
    ...
    (async () => {
        ...
        try {
            ...
            const newBody = $response.body.replace(/<body[^>]*>/, (bodyMatch) => `${bodyMatch}\n${html}`);
            $done({ body: newBody });
        } catch (err) {
