/*
 * 功能：重建 data.container.sections[*].item.tool.exContent.tools 数组（index === "5" 的项）
 */
...
if (obj && obj.data && obj.data.container && Array.isArray(obj.data.container.sections)) {
  obj.data.container.sections = obj.data.container.sections.map(section => {
    if (section.index === "5" && section.item && section.item.tool && section.item.tool.exContent) {
      section.item.tool.exContent.tools = [
        [
          {
